...
def parse_cards(cards):
    return [f"{c[1]}{c[0]}".upper() for c in cards]


def convert_actions(act, valid_actions):
    if "b" in act:
        return "raise", int(act[1:])
    elif "c" == act or "k" == act:
...
def split_actions(action_str):
    actions = []
    i = 0
    while i < len(action_str):
        if action_str[i] == "b":
            # Find the end of the bet amount
            j = i + 1
...
def generate_sb_action_and_amount(action, amount):
    if action == "fold":
        return "f"
    elif action == "call":
        if amount == 0:
...
def PlayHand(token):
    ...
    emulator.set_game_rule(
        player_num=2, max_round=1, small_blind_amount=50, ante_amount=0
    )

    # 2. Setup GameState object
    players_info = {
        "uuid-1": {"name": "player1", "stack": 20000},
        "uuid-2": {"name": "player2", "stack": 20000},
    }
    initial_state = emulator.generate_initial_game_state(players_info)
    game_state, events = emulator.start_new_round(initial_state)
    kane = Kane()
    kane.receive_round_start_message(0, [], players_info)
    ...
    while True:
        ...
        latest_action = r.get("action")[len(r["old_action"]) :].replace("/", "")
        ...
        for act in split_actions(latest_action):
            valid_actions = events[-1]["valid_actions"]
            p_action, p_amount = convert_actions(act, valid_actions)
            game_state, events = emulator.apply_action(game_state, p_action, p_amount)

        p_hole_cards = parse_cards(hole_cards)
        p_community_cards = parse_cards(board)
        valid_actions = events[-1]["valid_actions"]
        round_state = events[-1]["round_state"]
        round_state["community_card"] = p_community_cards

        kane_actions = kane.declare_action(valid_actions, p_hole_cards, round_state)
        incr = generate_sb_action_and_amount(kane_actions[0], kane_actions[1])
